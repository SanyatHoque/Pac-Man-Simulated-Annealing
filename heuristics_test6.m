function heuristics_test6
close all
clc
% heuristics_test with with node selection, ie. with T or L junction selection. 
...
enemies(1).dir = 1;                     % current ghost direction (right-1, down-2, left-3, up-4)
...
pacman.pos = [5 5]; %[25 8];%[27 30];%[25 8];%[7 30];      % position of pacman
...
%         enemies(2).status = 1;
        enemies(3).status = 1;
        pacman.pos = [5 5];%[25 8];%[27 30];%[25 8]; %[7 30];
        pacman.dir = 0;
        pacman.oldDir = 1;
        pacman.status = -2;
...
%         Test_pacman = square2
x11 = (square1(1)-square2(1)); %square1 = coordinates ghost
y12 = (square1(2)-square2(2)); %square2 = coordinates pacman
x1 = abs(square1(1)-square2(1)); %square1 = coordinates ghost
y1 = abs(square1(2)-square2(2)); %square2 = coordinates pacman
enemies(1).x1 = x1;
enemies(1).y1 = y1;
%         distance = x1 + y1
distance1 = x1 + y1 ;
enemies(1).distance1 = distance1;
test_square1 = [square1];  %Ghost coordinates
test_square2 = [square2];  %Pacman coordinates
x1a = (square1(1)-square2(1));
y1a = (square1(2)-square2(2));
test_square1 = square1;
test_square2 = square2;
test_x1a = x1a;
test_y1a = y1a;
enemies(1).x1a = x1a;
enemies(1).y1a = y1a;
...
t = linspace(1,10^3,50);  % t inversely proportional to Temperature, Therefore, Temperature Very Low-> Very High
% if (distance1 < 0);
%     Temperature = 10^3*((0.9910+0.0005)^t(-distance1)); % Temperature,Low->High % Arbitrary Max distance 50
% elseif (distance1 > 0);
    Temperature = 10^3*((0.9910+0.0005)^t(+distance1)); % Temperature,Low->High % Arbitrary Max distance 50
% else (distance1 == 0);
%     Temperature = 1;
...
% Test_Temperature = 10 ;
% if (distance1 <= 0);
%   sigmoid_function = (1 + exp(+06*((-distance1-(0))/Temperature)))^(-1);
% else (distance1 > 0);
    sigmoid_function = (1 + exp(+06*((1.5*distance1-(15))/Temperature)))^(-1);
% end
%%
...
% T_0 = 10^3;
% Temperature = T_0*(0.9910+0.0005)^t(distance1); % Temperature,Low->High % Arbitrary Max distance 50
% % end
% Temperature';
...
if (count1 == 1)
    ...
    test_dir = enemies(1).dir;
    % test_possibleMoves = enemies(1).possibleMoves
    ...
    if (corner==2) & (tunnel_out==0)  ;
        ...
        %         Test_oldDir = enemies(1).oldDir;
        %         Test_possibleMoves = enemies(1).possibleMoves
%         disp('Ghost is at a tunnel')
%% Node Selection End
...
%                 disp('Ghost at a L-shaped junction')
                flag100 = nextMove ;
            case 3   
%                 disp('T-junction')
                if x1 > y1 ;
...
%                 disp('Ghost at a T-shaped junction')
        end    
    end
    nextMove1 = nextMove;
...
for nnn = 1:h ;
Temperature_x(nnn) = T_01*(0.9910-0.0050)^tt(nnn); % Temperature,Low->High % Arbitrary Max distance 50
end
for nnn = 1:h ;
Temperature_x1(nnn) = T_01*(0.9910-0.0500)^tt(nnn); % Temperature,Low->High % Arbitrary Max distance 50
end
% Test_Temperature = Temperature_x' ;
figure
subplot(2,1,1)       % add first plot in 2 x 1 grid
hold on;
plot(1:nnn,Temperature_x,'-.r*','linewidth',2);
% plot(1:nnn,Temperature_x1,'-co','linewidth',2);
...
for ii = 1:kkk;
    sigmoid_function_x1(ii) = (1 + exp(06*(distance_x(ii)/Temperature_x1(ii))))^(-1);
%  Temperature_x(ii)
end
% sigmoid_function';
hold on;
threshold = 0.40 * ones(1,length(distance_x)); 
subplot(2,1,2)       % add second plot in 2 x 1 grid
hold on;
plot(1:ii,sigmoid_function_x,':r*','linewidth',2); % probability - y axis;k - x axis; 
% plot(1:ii,sigmoid_function_x1,'-co','linewidth',2); % probability - y axis;k - x axis;
