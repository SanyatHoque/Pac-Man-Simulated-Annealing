...
%         Test_nextMove = nextMove;
        ...
        corner = numel(possibleMoves);
    if any(corner==1) && ~any(possibleMoves==enemies(4).oldDir);%&& numel(enemies(1).possibleMoves)==numel(enemies(1).possibleMoves);
      nextMove = enemies(4).dir;  
    elseif any(corner > 1) && ~any(possibleMoves==enemies(4).oldDir) %&& numel(enemies(1).possibleMoves)==numel(enemies(1).possibleMoves);
%       nextMove = nextMove;  
%         msize = numel(possibleMoves) ;
%         idx = randperm(msize) ;
%         nextMove = possibleMoves(idx(1:1)) ;
