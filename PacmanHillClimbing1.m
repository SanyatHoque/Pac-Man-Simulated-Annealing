function PacmanHillClimbing1
%% Genetical Algorithm based Behaviour Tree is based on Fitness of Time it takes for Bots to complete Flanking attack as well as
%% either - Pincer movement,Encirclement or Aggressive Attack based on Temperature and Acceptance Pr. of Sigmoid Function
%% Fittest Strategy based on a Behaviour Tree is chosen based on fastest implementation of above Actions
% Please Note : Timer parameters are present for ONGOING work to optimise behaviour tree to identify
% fittest strategy and further mutate or crossover strategies to find
% optimal strategy based on Genetic Algorithm 
...
%      z1 = tic  ;
     tic ;
        Timer_on1=0 ; 
    elseif (Timer_on1==0) && (enemies(1).distance1 < 5) ;            
        Timer_1 = toc
        Timer_on1 = 1  ;
    end
% End Timer
...
%     % k = numel(distance);
%     sigmoid_function = (1 + exp(distance1/Temperature))^(-1) ;
%     % sigmoid_function';
...
% %       flag99091 = 1
%     Pincer_Moves = [0];
%     msize = numel(Pincer_Moves) ;
%     idx = randperm(msize) ;
%     nextMove1 = Pincer_Moves ;  %(idx(1:1)) ;
%    % disp('Pincer_Move - Attack or Blockade jounction') 
% end
end
%%
%%
...
%     % del = linspace(-20,120,100);
%     % k = numel(distance);
%     sigmoid_function = (1 + exp(distance2/Temperature))^(-1) ;
%     % sigmoid_function';
%     % plot(1:i,sigmoid_function) % probability - y axis; k - x axis;
%     count1 = 0 ;
%     if (sigmoid_function <= 0.498);
...
% elseif (count1 == 0)
% %       flag99091 = 1
%     Pincer_Moves = [possibleMoves];
%     msize = numel(Pincer_Moves) ;
%     idx = randperm(msize) ;
%     nextMove2 = Pincer_Moves(idx(1:1)) ;
%    % disp('Pincer_Move - Attack or Blockade jounction')
% end
end
%%
...
%     % del = linspace(-20,120,100);
%     % k = numel(distance);
%     sigmoid_function = (1 + exp(distance3/Temperature))^(-1) ;
%     % sigmoid_function';
%     % plot(1:i,sigmoid_function) % probability - y axis; k - x axis;
%     count1 = 0 ;
%     if (sigmoid_function <= 0.498);
...
%     msize = numel(Pincer_Moves) ;
%     idx = randperm(msize) ;
%     nextMove3 = Pincer_Moves(idx(1:1)) ;
%    % disp('Pincer_Move - Attack or Blockade jounction')
% end
end
%%
...
%     msize = numel(Pincer_Moves) ;
%     idx = randperm(msize) ;
%     nextMove4 = Pincer_Moves ;  %(idx(1:1)) ;
%    % disp('Pincer_Move - Attack or Blockade jounction')
% end
end
%%
...
% for ii = 1:kkk;
%     sigmoid_function_x(ii) = (1 + exp(+distance_x(ii)/Temperature_x(ii)))^(-1);
% %  Temperature_x(ii)
% end
% % sigmoid_function';
% subplot(2,1,2)       % add second plot in 2 x 1 grid
% plot(1:ii,sigmoid_function_x) % probability - y axis; k - x axis; 
% title('Sigmoid Function, Acceptance Pr > x_0')
